...
async function applyApprovedRequestCorrection(
  ...
  if (requestTypeUpper.includes('FORGOT') && (requestTypeUpper.includes('BREAK') || requestTypeUpper.includes('LUNCH'))) {
    ...
    // Build the interval in Colombia business time so the break segment and the
    // session lookup use the correct local day, not the server timezone.
    let breakInterval: BusinessBreakInterval
    try {
      breakInterval = buildBusinessBreakInterval({ date: breakDate, timeFrom, timeTo })
    } catch (intervalError) {
      ...
      return correctionError('Cannot approve request because the break/lunch time range is invalid')
    }

    // Find the time_session for this Colombia business date.
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    // Validate time-entry edits (overlap / bounds checks) BEFORE applying the
    // correction, so an invalid edit is never partially applied.
    ...
    // Apply the approved correction exactly once (single application path).
    if (status === 'APPROVED') {
